...
先读事件产品入口 [SKILL.md](../SKILL.md) 的命令规则、生命周期和失败处理。本参考覆盖员工转正与调岗两个 HR 个人事件。

<!-- dws-intent: event.listen.hr -->员工转正或调岗生命周期变化必须使用 `dws event consume` 长连接，不轮询员工档案或审批记录模拟事件。
...
DWS CLI 仍通过统一控制面创建和取消个人订阅。DWS 后端识别这两个 eventKey 后调用 HR 预发 HSF Provider：
...
- `dws.event.source.conf`：`category=HRM`，两个 eventKey 均声明 `eventSource=hrm`。

...
- **实时个人事件例外**：普通 IM 消息、reaction、已读和撤回默认走 `dws event +listen-im ...`；OA 审批、HR 生命周期、群生命周期、明确的原始 EventKey、Filter DSL、subscribe_id 或原始 envelope 使用 `dws event consume ... --flatten`。不要用轮询模拟事件
...
| `event`           | 个人 IM/OA/HR 事件：监听消息、群生命周期、审批任务/实例、员工转正/调岗/入职/离职事件，NDJSON 输出 | [event.md](./references/products/event.md)                     |
...
用户提到"监听员工转正/监听转正状态变化/监听员工调岗/监听岗位调动/监听员工入职/监听员工离职/订阅 HR 生命周期事件/event consume user_hrm_*" → `event consume`

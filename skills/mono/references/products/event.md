...
| `dws event list --category hr` | 查看当前公开的 HR 生命周期事件目录 |
...
“我和某人的单聊”使用 `receive_o2o`；“某人发给我的消息”使用 `receive_user`。只有明确说“所有”时才使用全量事件。执行撤回走 `dws chat`，监听撤回才走 `dws event`。OA/HR 事件不接受 `--user`、`--open-dingtalk-id`、`--group`、`--query` 或 `--filter-json`。

## Call flow

1. 普通 IM 意图选择 `+listen-im`；OA、HR 与高级事件控制手选 EventKey。
2. 需要了解字段时运行 `dws event schema <event_key> --flatten`，读取 `schema.properties`；此模式的 `jq_root_path` 为 `.`。
3. 普通 IM 启动 `+listen-im`；OA、HR 或高级任务启动 `event consume ... --flatten -f ndjson`。等待真实 ready marker，不用 `sleep` 猜测。
4. stdout 每行一个扁平事件 JSON；HR 当前只读取公共字段和 `payload` 中实际存在的业务字段。
...
- HR 事件当前只承诺 `type/event_id/timestamp/subscribe_id/payload`；不猜测员工、岗位、部门、状态或操作者字段。字段契约补齐后再升级强类型输出。
...
- HR reference: `skills/multi/dingtalk-event/references/event-hr.md`

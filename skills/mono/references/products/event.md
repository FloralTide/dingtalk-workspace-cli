...
- VoIP 事件读取顶层 `biz_id/corp_id/org_id/target_uid/call_id/caller_uid/callee_uid/call_type/room_id/sdk_app_id/sdk_expire_time/create_time/event_time`。`caller_uid` 读取新的 `body.callerUserId` 并兼容旧 `body.callerUid`；它与 `callee_uid` 都是字符串标识。默认输出会移除 `roomCode` 和 header `X-Mozi-Agent-Sdk-Token`；数字员工确需动态 MeetingSDK 凭证时显式加 `--include-voip-sdk-token`，并将 stdout 直接交给受控入会进程，不得记录或转发。
